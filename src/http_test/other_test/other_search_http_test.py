...
def test_search_multiple_messages(reset, new_user, new_channel, send_msg):
    ...
    search_a = requests.get(f'{BASE_URL}/search', params=search_input_a).json()
    search_b = requests.get(f'{BASE_URL}/search', params=search_input_b).json()
    search_romeo = requests.get(f'{BASE_URL}/search',
                                params=search_input_romeo).json()
